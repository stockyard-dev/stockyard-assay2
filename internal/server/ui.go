...
async function load(){var r=await fetch(A+'/contracts').then(function(r){return r.json()});items=r.contracts||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.endpoint||'').toLowerCase().includes(q)||(i.method||'').toLowerCase().includes(q)||(i.expected_status||'').toLowerCase().includes(q)});
...
if(i.endpoint)h+='<div class="item-sub">'+esc(i.endpoint)+'</div>';
...
if(i.method)h+='<span>'+esc(i.method)+'</span>';
if(i.expected_status)h+='<span>'+esc(i.expected_status)+'</span>';
if(i.expected_body)h+='<span>'+esc(i.expected_body)+'</span>';
if(i.headers)h+='<span>'+esc(i.headers)+'</span>';
if(i.last_result)h+='<span>'+esc(i.last_result)+'</span>';
if(i.last_run_at)h+='<span>'+esc(i.last_run_at)+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';

h+='</div>';});document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/contracts/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",endpoint:"",method:"",expected_status:"",expected_body:"",headers:"",last_result:"",last_run_at:""};var isEdit=!!item;
...
h+='<div class="fr"><label>Method</label><input id="f-method" value="'+esc(i.method)+'"></div>';
h+='<div class="fr"><label>Expected Status</label><input id="f-expected-status" value="'+esc(i.expected_status)+'"></div>';
h+='<div class="fr"><label>Expected Body</label><input id="f-expected-body" value="'+esc(i.expected_body)+'"></div>';
h+='<div class="fr"><label>Headers</label><input id="f-headers" value="'+esc(i.headers)+'"></div>';
h+='<div class="fr"><label>Last Result</label><input id="f-last-result" value="'+esc(i.last_result)+'"></div>';
h+='<div class="fr"><label>Last Run At</label><input id="f-last-run-at" value="'+esc(i.last_run_at)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),endpoint:document.getElementById('f-endpoint').value.trim(),method:document.getElementById('f-method').value.trim(),expected_status:document.getElementById('f-expected-status').value.trim(),expected_body:document.getElementById('f-expected-body').value.trim(),headers:document.getElementById('f-headers').value.trim(),last_result:document.getElementById('f-last-result').value.trim(),last_run_at:document.getElementById('f-last-run-at').value.trim()};
if(editId){await fetch(A+'/contracts/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/contracts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
