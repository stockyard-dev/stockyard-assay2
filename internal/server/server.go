package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-assay2/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-assay2"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/contracts",s.handleList)
    s.mux.HandleFunc("POST /api/contracts",s.handleCreate)
    s.mux.HandleFunc("DELETE /api/contracts/{id}",s.handleDelete)
    s.mux.HandleFunc("POST /api/contracts/{id}/run",s.handleRun)
    s.mux.HandleFunc("GET /api/contracts/{id}/results",s.handleResults)
